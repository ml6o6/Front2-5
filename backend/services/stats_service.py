...
def by_type(db: Session) -> list[dict]:
    rows = (
        db.query(Accident.accident_type.label("type"), func.count(Accident.id).label("count"))
...
def by_cause(db: Session) -> list[dict]:
    rows = (
        db.query(Accident.accident_cause.label("cause"), func.count(Accident.id).label("count"))
