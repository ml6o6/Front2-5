...
def seeded(client, admin_headers):
    """Создаёт авто, водителя и 5 ДТП с известным распределением:
    3 столкновения, 2 наезда на пешехода. Все в июне 2025.
    """
    client.post(
        "/api/cars",
...
def test_by_type(client, user_headers, seeded):
    r = client.get("/api/stats/by-type", headers=user_headers)
    assert r.status_code == 200
    rows = {row["type"]: row["count"] for row in r.json()}
    assert rows["Столкновение"] == 3
    assert rows["Наезд на пешехода"] == 2


def test_by_type_sorted_descending(client, user_headers, seeded):
    rows = client.get("/api/stats/by-type", headers=user_headers).json()
...
def test_by_day(client, user_headers, seeded):
    r = client.get("/api/stats/by-day?year=2025&month=6", headers=user_headers)
    assert r.status_code == 200
    rows = {row["day"]: row["count"] for row in r.json()}
    assert rows[1] == 2  # два ДТП 1 июня
    assert rows[5] == 1
    assert rows[10] == 1
    assert rows[15] == 1
...
def test_by_location(client, user_headers, seeded):
    rows = client.get("/api/stats/by-location?limit=10", headers=user_headers).json()
    by_loc = {r["location"]: r["total_victims"] for r in rows}
    # ул. Ленина, 1: 2 + 1 + 3 = 6 потерпевших
    assert by_loc["ул. Ленина, 1"] == 6
    assert by_loc["пр-т Мира, 10"] == 0
    assert by_loc["Тверская, 7"] == 1
    # Сортировка по убыванию числа потерпевших
    victims = [r["total_victims"] for r in rows]
...
def test_summary(client, user_headers, seeded):
    r = client.get("/api/stats/summary", headers=user_headers)
    assert r.status_code == 200
    data = r.json()
    assert data["total_accidents"] == 5
    assert data["total_victims"] == 7  # 2+1+0+3+1
    assert data["top_type"] == "Столкновение"  # 3 раза
