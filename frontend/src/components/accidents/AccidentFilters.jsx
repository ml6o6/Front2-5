// frontend/src/components/accidents/AccidentFilters.jsx
const TYPES = [
...
export default function AccidentFilters({ value, onChange, onReset }) {
  ...
  return (
    <div className="filters">
      ...
      <label>
        <span>Вид ДТП</span>
        <select value={v.accident_type || ''} onChange={(e) => set('accident_type', e.target.value)}>
      ...
      <label>
        <span>Причина</span>
        <select value={v.accident_cause || ''} onChange={(e) => set('accident_cause', e.target.value)}>
          <option value="">Все</option>
      ...
      <label>
        <span>Место</span>
        <input value={v.location || ''} onChange={(e) => set('location', e.target.value)} placeholder="ул., адрес…" />
      </label>
      <button className="btn btn--ghost" onClick={onReset}>Сбросить</button>
    </div>
  );
}
