// frontend/src/components/drivers/DriverForm.jsx
...
export default function DriverForm({ initial, onSubmit, onCancel }) {
  ...
  useEffect(() => {
    if (initial) setForm({ ...EMPTY, ...initial, car_reg_number: initial.car_reg_number || '', act_number: initial.act_number || '' });
    else setForm(EMPTY);
  }, [initial]);
  ...
  return (
    <form className="form" onSubmit={submit}>
      ...
      <label>
        <span>Стаж (лет)</span>
        <input type="number" min="0" max="80" value={form.experience} onChange={(e) => change('experience', e.target.value)} />
      </label>
      <label>
      ...
      <label>
        ...
        <input required type="date" value={form.license_date} onChange={(e) => change('license_date', e.target.value)} />
      </label>
      <label>
        <span>№ акта о ДТП</span>
