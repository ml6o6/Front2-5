// frontend/src/components/cars/CarForm.jsx
import { useEffect, useState } from 'react';

const EMPTY = { brand_company: '', brand_model: '', body_type: 'седан', reg_number: '' };

const BODY_TYPES = ['седан', 'хэтчбек', 'универсал', 'внедорожник', 'кроссовер', 'купе', 'минивэн', 'пикап'];
  ...
  return (
    <form className="form" onSubmit={submit}>
      ...
      <label><span>Фирма (производитель)</span>
        <input required value={form.brand_company} onChange={(e) => change('brand_company', e.target.value)} />
      </label>
      <label><span>Марка (модель)</span>
        <input required value={form.brand_model} onChange={(e) => change('brand_model', e.target.value)} />
      </label>
      ...
      <label><span>Гос. номер</span>
        <input required value={form.reg_number} onChange={(e) => change('reg_number', e.target.value.toUpperCase())} />
      </label>
