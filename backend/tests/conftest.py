...
def client(engine):
    ...
    # Seed admin + user
    s = TestingSessionLocal()
    s.add(User(username="admin", password_hash=hash_password("admin123"), role=UserRole.admin, is_active=True))
    s.add(User(username="user", password_hash=hash_password("user123"), role=UserRole.user, is_active=True))
    s.commit()
    s.close()
