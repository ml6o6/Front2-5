...
def list_drivers(search: str | None = None, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def get_driver(driver_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    history = driver_service.driver_accident_history(db, driver_id)
    response = DriverDetailResponse.model_validate(driver)
    response.accidents = [DriverAccidentHistoryItem.model_validate(a) for a in history]
    return response


@router.post("", response_model=DriverResponse, status_code=201)
def create_driver(payload: DriverCreate, db: Session = Depends(get_db), _=Depends(require_admin)):
