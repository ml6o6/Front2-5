...
def create_car(db: Session, payload: CarCreate) -> Car:
    if db.query(Car).filter(Car.reg_number == payload.reg_number).first():
        raise HTTPException(status_code=400, detail="Авто с таким гос. номером уже существует")
...
def update_car(db: Session, car_id: int, payload: CarUpdate) -> Car:
    ...
    # Если меняется reg_number — нужно каскадно обновить все ссылки,
    # т.к. в БД нет ON UPDATE CASCADE.
    new_reg = data.get("reg_number")
    old_reg = car.reg_number
    if new_reg and new_reg != old_reg:
        # Проверяем, что новый номер ещё не занят
        existing = db.query(Car).filter(Car.reg_number == new_reg, Car.id != car_id).first()
        if existing:
            raise HTTPException(
                status_code=400,
                detail=f"Авто с гос. номером «{new_reg}» уже существует",
            )
        # Создаём временную запись с новым reg_number,
        # переключаем все FK на неё, потом удаляем старую.
        # Самый простой способ — вставить новую и перенаправить ссылки.
        # Но проще: вставить новый Car, перенаправить ссылки, удалить старый.
        new_car = Car(
        ...
        db.add(new_car)
        db.flush()  # чтобы reg_number стал доступен для FK

        # Обновляем все ссылки
        db.execute(
            update(Driver)
            .where(Driver.car_reg_number == old_reg)
            .values(car_reg_number=new_reg)
        )
        db.execute(
            update(Accident)
            .where(Accident.car_reg_number == old_reg)
            .values(car_reg_number=new_reg)
        )
        db.execute(
            update(AccidentCar)
            .where(AccidentCar.car_reg_number == old_reg)
            .values(car_reg_number=new_reg)
        )
    ...
    # Reg_number не меняется — обычное обновление
