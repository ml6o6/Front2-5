...
def _normalize_refs(data: dict) -> dict:
    """Превращает пустые строки во внешних ключах в None."""
...
def create_driver(db: Session, payload: DriverCreate) -> Driver:
    if db.query(Driver).filter(Driver.license_number == payload.license_number).first():
        raise HTTPException(status_code=400, detail="Удостоверение с таким номером уже существует")
    data = _normalize_refs(payload.model_dump())
...
def update_driver(db: Session, driver_id: int, payload: DriverUpdate) -> Driver:
    ...
    data = _normalize_refs(payload.model_dump(exclude_unset=True))
    _validate_refs(
        db,
