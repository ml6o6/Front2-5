...
def list_cars(search: str | None = None, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def update_car(car_id: int, payload: CarUpdate, db: Session = Depends(get_db), _=Depends(require_admin)):
