...
def test_list_users_returns_role_and_active_flag(client, admin_headers):
...
def _find_user_id(client, admin_headers, username: str) -> int:
    rows = client.get("/api/users", headers=admin_headers).json()
...
def test_set_role_requires_auth(client, admin_headers):
    uid = _find_user_id(client, admin_headers, "user")
    r = client.patch(f"/api/users/{uid}/role", json={"role": "admin"})
    assert r.status_code == 401
